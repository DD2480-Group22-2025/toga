...
class Font:
    def __init__(self, interface):
        ...
        try:
            ...
            elif isinstance(self.interface.size, str) and self.interface.size in ABSOLUTE_FONT_SIZES:
                base_size = NSFont.systemFontSize                
                font_size = base_size * FONT_SIZE_SCALE.get(self.interface.size, 1.0)
            elif isinstance(self.interface.size, str) and self.interface.size in RELATIVE_FONT_SIZES:    
                parent_size = getattr(self.interface, "_parent_size", NSFont.systemFontSize)
                font_size = parent_size * RELATIVE_FONT_SIZE_SCALE.get(self.interface.size, 1.0)
            else:
